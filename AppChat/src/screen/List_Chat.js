...
const List_Chat = () => {
    ...
    const fetchChats = async () => {
      try {
        ...
        const fetchMessagesPromises = snapshot.docs.map(async (chatDoc) => {
          ...
          // Check if UID exists and is an array
          if (Array.isArray(chatData.UID)) {
            const chatUIDs = chatData.UID.filter((uid) => uid !== user.uid);
            
            // Check if there's at least one other UID or user's own UID is in the array
            if (chatUIDs.length > 0 || chatData.UID.includes(user.uid)) {
              const otherUID = chatUIDs[0] || user.uid; // Use user's own UID if no other UID found
              const userDocRef = doc(db, 'users', otherUID);
              const userDocSnap = await getDoc(userDocRef);
              ...
              // Handle case where there's no other UID and user's own UID is not in the array
              console.log('No other UID in array:', chatData.UID);
            }
          }
        });
  ...
  const renderItem = ({ item }) => (
    <Pressable
      ...
      onPress={() => navigation.navigate("Chat_messages", { friendData: item.otherUser, ID_room1: item.ID_room, chatData: item })}
